...
export default function Main() {
    ...
    return (
      <div id='main-top-wrapper'>
        <div className='main-top-container'>
          <div className='main-top-intro'>{'Sooin\'s Board에서\n다양한 이야기를 나눠보세요.'}</div>
  ...
  const MainBottom = () => {
    ...
    const [currentBoardList, setCurrentBoardLits] = useState<BoardListItem[]>([]);
    //          state: 인기 검색어 리스트 상태         //
    const [popularWordList, setPopularWordLits] = useState<String[]>([]);

    //          effect: 첫 마운트 시 실행 될 함수          //
    useEffect(() => {
      setCurrentBoardLits(latestBoardListMock);
      setPopularWordLits(['안녕', '잘가', '또봐']);
    }, []);
